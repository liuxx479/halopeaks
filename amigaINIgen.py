...
##################
def create_ini_file (snap_ID):
	f = open('/work/02977/jialiu/lenstools_home/amiga_dir/amiga_%i.ini'%(snap_ID), 'w')#open('amigatest.txt','w')#
	content = '''[AHF]

# (stem of the) filename from which to read the data to be analysed
ic_filename       = snapshot_{0:03d}.

# what type of input file (cf. src/libio/io_file.h)
ic_filetype       = 61

# prefix for the output files
outfile_prefix    = amiga/snap{0:d}
...
[ART]
ART_BOXSIZE         = 20
ART_MUNIT           = 6.5e8'''.format(snap_ID)

	f.write(content)
	f.close()
	print 'Create new amiga.ini for snap%i.'%(snap_ID)
	
map(create_ini_file, range(60))
